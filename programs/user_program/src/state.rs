...
impl UserAccount {
    ...
    pub fn maybe_credit_weekly_reward(&mut self, now: i64) -> bool {
        if self.is_demo && (now - self.last_weekly_reward_ts >= Self::WEEK_SECONDS) {
            // #64: Log a warning on overflow instead of silently saturating
            match self.demo_balance.checked_add(Self::WEEKLY_REWARD) {
                Some(new_balance) => self.demo_balance = new_balance,
                None => {
                    msg!("WARNING: demo_balance overflow for user, saturating at u64::MAX");
                    self.demo_balance = u64::MAX;
                }
            }
